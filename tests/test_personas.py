...
class TestEngineIsolation(unittest.TestCase):
    ...
    def test_preflight_does_not_import_engine_or_model_client(self):
        """cadre/preflight.py must NOT import engine or model_client (U4, #62, R7).

        The #62 preflight-refuse gate is read-only config/palette inspection
        performed by the two runners before they spend anything — same
        caller-layer posture as approval.py/provision.py/verify_palette.py.
        """
        import cadre.preflight as pf_mod

        imported = _static_imports(pf_mod)
        self.assertNotIn(
            "cadre.engine",
            imported,
            "preflight.py must not import engine (engine-purity constraint)",
        )
        self.assertNotIn(
            "cadre.model_client",
            imported,
            "preflight.py must not import model_client (engine-purity constraint)",
        )

    def test_engine_does_not_import_preflight(self):
        """cadre/engine.py and model_client.py must NOT import preflight (U4, #62)."""
        import cadre.engine as e_mod
        import cadre.model_client as mc_mod
        ...
        self.assertNotIn(
            "cadre.preflight",
            imported_engine,
            "engine.py must not import preflight (engine-purity constraint)",
        )
        imported_mc = _static_imports(mc_mod)
        self.assertNotIn(
            "cadre.preflight",
    ...
    def test_palette_fleet_does_not_import_engine_or_model_client(self):
        """cadre/palette_fleet.py must NOT import engine or model_client (#61, U5).

        palette_fleet.py generates the smoke-test fleet from verified palette
        records — caller-layer, same posture as verify_palette.py/discover.py.
        It imports cadre.capture (for resolved_hermes_home), which itself
        imports engine — but that is a transitive dependency of capture.py,
        not a direct import in palette_fleet.py's own source, so it does not
        trip this AST check.
        """
        import cadre.palette_fleet as pf_mod

        imported = _static_imports(pf_mod)
        self.assertNotIn(
            "cadre.engine",
            imported,
            "palette_fleet.py must not import engine (engine-purity constraint)",
        )
        self.assertNotIn(
            "cadre.model_client",
            imported,
            "palette_fleet.py must not import model_client (engine-purity constraint)",
        )

    def test_engine_does_not_import_palette_fleet(self):
        """cadre/engine.py and model_client.py must NOT import palette_fleet (#61, U5)."""
        import cadre.engine as e_mod
        import cadre.model_client as mc_mod
        ...
        self.assertNotIn(
            "cadre.palette_fleet",
            imported_engine,
            "engine.py must not import palette_fleet (engine-purity constraint)",
        )
        imported_mc = _static_imports(mc_mod)
        self.assertNotIn(
            "cadre.palette_fleet",
