...
4. **Install `cadre` into that Python.** No clone required — this is the load-bearing install target (the only interpreter that can import both `cadre` and `run_agent`):
   ...
   ```
   Pin `<ref>` to a tag or commit. **A bare `"$PYBIN" -m pip install cadre` grabs a different, unrelated package on PyPI** — always use the git URL above (`cadre` hasn't published under its own name yet). `--force-reinstall` matters on every reinstall/upgrade: pip no-ops an already-installed same-version dist, so a bare `pip install git+…@branch` on an unchanged version silently re-tests stale bytes. `--no-deps` **skips** installing `cadre`'s dependencies (just `pyyaml`) — safe on a Hermes venv, which already carries `pyyaml` (Hermes itself uses YAML); on a venv that lacks it, drop `--no-deps` (or `pip install pyyaml` first), or `cadre` won't import.
...
6. **Provision `~/.cadre` and verify the palette:**
   ...
   ```
   Scaffolds `~/.cadre` owner-only, seeds all seven starter fleets + personas + a `palette-candidates.yaml` from the installed package (no repo-tree reads), and records `$PYBIN` to `~/.cadre/config`. Edit `~/.cadre/palette-candidates.yaml` to your authenticated `(provider, model)` pairs and declared toolsets, then verify live:
   ```bash
   "$PYBIN" -m cadre.cli verify-palette
   ```
   Confirms provider inheritance (no api_key needed) and writes `~/.cadre/palette.yaml` — the menu an agent composes fleets from. **Tool invocation is profile-dependent** — a `web`/`x_search` lane only fires if the *active profile* (b) has the tool creds, so run `verify-palette` (and edit the candidates' `toolsets`) under the profile you'll run the fleet in. **If provider inheritance is refuted, stop** and revisit the adapter.
...
8. **Edit a seeded fleet.** `cadre setup` already seeded all seven starters into `~/.cadre/fleets/` (e.g. `research-swarm.yaml`) — open it and set the confirmed provider/model strings from `~/.cadre/palette.yaml`; never commit real keys/tokens.
...
**First run** — installs `cadre` into the resolved venv (`pip install --force-reinstall --no-deps .`), runs `cadre setup` to scaffold `~/.cadre/{,fleets/}` owner-only (`0o700`), seed starter fleets/personas and `~/.cadre/palette-candidates.yaml` from the installed package (no repo-tree reads), and record the resolved venv Python to `~/.cadre/config` (`0o600`), then **stops** with a message to edit the candidates file.

**Second run** (after editing candidates) — runs `cadre verify-palette` under the Hermes venv to confirm which `(provider, model)` pairs actually resolve and respond, then writes the verified palette to `~/.cadre/palette.yaml`. Also installs the `cadre-fleet` skill symlink via `cadre install-skill` if `HERMES_SKILLS_DIR` is set.
...
2. **Re-run to verify:** `./scripts/install.sh` (or `"$PYBIN" -m cadre.cli verify-palette` directly) — verifies live and writes `~/.cadre/palette.yaml`.
3. **Edit a seeded fleet:** `cadre setup` already seeded `~/.cadre/fleets/research-swarm.yaml` — open it and set provider/model strings from the palette.
