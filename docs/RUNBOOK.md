...
6. **Provision `~/.cadre` and verify the palette:**
   ...
   ```
   Scaffolds `~/.cadre` owner-only, seeds all seven starter fleets + personas, and records `$PYBIN` to `~/.cadre/config`. It also seeds `~/.cadre/policy.yaml` — a fully-commented, inert-by-default local allow/deny gate: leave it as-is for zero behavior change, or uncomment `deny_providers`/`restrict_models` rules to block a provider outright or pin an expensive model family to the one provider whose billing you intend. Enforced at `cadre discover` (excludes banned pairs), `cadre verify-palette` (refuses to admit — and prunes an already-admitted pair that a tightened policy now bans), and preflight (refuses a fleet referencing a banned pair, `exit 5`, before any spend — even if that pair is on the palette). A malformed policy file fails every one of those closed rather than silently disabling itself. The `palette-candidates.yaml` seeding step **auto-discovers** your authenticated providers straight from Hermes's own inventory (`hermes_cli.inventory`, read in-process — zero-cost, no model call) when that package is importable in this venv; otherwise it falls back to a placeholder file and says why on stderr. Either way, verify live:
   ```bash
   "$PYBIN" -m cadre.cli verify-palette   # or --all to verify every discovered candidate, not just the capped default
   ```
   By default this verifies a capped subset (2 candidates per provider, in file order) rather than everything discovered — a real host can surface dozens of candidates across a handful of providers, and paying for every one before the palette is even usable is needless spend; it prints the X-of-Y split before the first paid call. Previously-verified pairs still in the candidates file are always re-verified (exempt from the cap), so a routine re-verify never silently shrinks an existing palette; pairs that left the candidates file entirely are named in a pre-spend warning before they drop. Want to trim or reorder candidates first? Hand-edit `~/.cadre/palette-candidates.yaml` before running this — that edit survives until the next `cadre discover` (which regenerates the file and discards it) — and declare the `toolsets` your profile holds there too. Confirms provider inheritance (no api_key needed) and writes `~/.cadre/palette.yaml` — the menu an agent composes fleets from. On success with 2+ providers verified, it also (re)generates `~/.cadre/fleets/palette-fleet.yaml` — a tool-less smoke-test fleet, one lane per verified provider, runnable with zero further editing. **Tool invocation is profile-dependent** — a `web`/`x_search` lane only fires if the *active profile* (b) has the tool creds, so run `verify-palette` (and edit the candidates' `toolsets`) under the profile you'll run the fleet in. **If provider inheritance is refuted, stop** and revisit the adapter.
