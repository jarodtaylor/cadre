...
- [Native tool use is invisible to dispatch-side detection](../best-practices/verify-tool-use-by-effect-not-dispatch-signal.md) — the *other* grounding-blindness axis: even with the tool provisioned, you cannot mechanically confirm it fired (a provider's native web search leaves no tool-call trace), so verify the output, not the dispatch.
