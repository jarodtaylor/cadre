...
- [[verify-tool-use-by-effect-not-dispatch-signal]] (`docs/solutions/best-practices/verify-tool-use-by-effect-not-dispatch-signal.md`) — live per-toolset verification (GitHub #48) was investigated and found not mechanically achievable; even if it worked it would only confirm a tool is *available* at runtime, not catch an underspecified focus — runtime verification and focus design are separate guards.
