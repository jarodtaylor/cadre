...
def _ok_and_detail(result: AgentResult, captured: str) -> tuple[bool, str]:
    """Map the adapter's typed result onto verify's ``(ok, detail)`` contract.

    Classification itself happened in ``ModelClient.run`` (#76) — this only
    derives the display detail. EMPTY_OUTPUT keeps the pre-#76 behavior of
    mining the suppressed provider output for a calm one-line reason (AIAgent
    logs a dead provider and returns nothing usable, so the typed error alone
    is just "empty response"); a flagged or raised failure already carries its
    own structured detail in ``result.error``.
    """
    if result.ok:
        return True, str(result.text)[:60]
    if result.reason is FailureReason.EMPTY_OUTPUT:
        return False, _reason_from_capture(captured) or "empty response"
    return False, result.error or "call failed"


def _verify_one(provider: str, model: str) -> tuple[bool, str]:
    """Run one verification call, SILENCING the provider's own output.

    The call AND its classification are delegated to the engine's own adapter
    (``ModelClient.run`` → ``run_conversation`` structured flags, #76) so
    verification can never diverge from what a fleet lane experiences: a result
    AIAgent flags as failed can never verify a pair (the U9 false-verify shape
    — error text returned as response text — is exactly what this closes),
    while a response merely QUOTING an error message with clean flags still
    passes. ``_agent`` is injected as the factory, so tests keep patching it
    and the ``run_agent`` import stays lazy and host-only.
    ...
    Returns ``(ok, detail)``; never raises (``ModelClient.run``'s contract —
    any exception, including agent construction, comes back as a typed
    MODEL_ERROR whose ``error`` is the same ``TypeName: message`` string the
    pre-#76 except-branch produced here).
    """
    def _call() -> AgentResult:
        return ModelClient(agent_factory=_agent).run(
            role="verify", provider=provider, model=model,
            prompt="Reply with the single word: ok", toolset=[],
        )
    ...
    if os.getenv("CADRE_VERIFY_VERBOSE"):
        ...
        # captured="" — verbose mode surfaces the provider output itself, so the
        # detail never mines it (pre-#76 verbose behavior: plain "empty response").
        return _ok_and_detail(result, captured="")

    # Default: capture the provider's output + mute logging so a skip is one calm
    # line — then let _ok_and_detail mine the capture when the typed result alone
    # carries no reason (the empty-output shape).
    sink = io.StringIO()
    logging.disable(logging.CRITICAL)
    try:
        with contextlib.redirect_stdout(sink), contextlib.redirect_stderr(sink):
            result = _call()
        return _ok_and_detail(result, sink.getvalue())
    finally:
...
def write_palette(
    ...
    # a no-tool call). A model can resolve while its search tool is unprovisioned,
    ...
    header = (
        ...
        "# models:   provider/model pairs confirmed by a live model call.\n"
