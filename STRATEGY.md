...
3. **Trust & safety hardening.** Make the human-gate's preview-binding *code-enforced* — a run executes only when it's bound to its own preview (what runs is what was previewed, one-shot and fail-closed); human *presence* at the okay stays procedural in this single-operator deployment. Also on this track (not yet shipped): live per-toolset verification, so a lane can't answer ungrounded from an unprovisioned toolset — attempted, but the naive probe proved unreliable, tracked as [#48](https://github.com/jarodtaylor/cadre/issues/48). *Serves the "human approves before execution" commitment as an enforced binding, not a presence proof.*
