...
3. **Trust & safety hardening.** Make the human-gate *bulletproof* — non-forgeable preview-bound approval + live per-toolset verification (safe toolsets still read untrusted web, and the synthesis is consumed by a terminal-capable agent). *Serves the "human approves before execution" commitment directly.*
