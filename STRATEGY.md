...
3. **Trust & safety hardening.** Make the human-gate's preview-binding *code-enforced* — a run executes only when it's bound to its own preview (what runs is what was previewed, one-shot and fail-closed); human *presence* at the okay stays procedural in this single-operator deployment. Live per-toolset verification (so a lane can't answer ungrounded from an unprovisioned toolset) was attempted on this track, but a source-level investigation ([#48](https://github.com/jarodtaylor/cadre/issues/48)) found that **no mechanical tool-fire signal Hermes exposes** separates native grounding from a memory answer — a provider's server-side web search grounds with no detectable tool-call — so the palette stays at an honest declared-and-warned baseline. *Serves the "human approves before execution" commitment as an enforced binding, not a presence proof.*
