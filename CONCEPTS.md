...
### Palette discovery
The free `cadre discover` step — also run automatically by `cadre setup` when possible — that reads Hermes's own authenticated-provider inventory in-process and writes every `(provider, model)` pair it finds to `~/.cadre/palette-candidates.yaml`, the same file the verify step already reads. Discovery optimizes for recall: it writes down everything Hermes reports as authenticated; the live verify call, not discovery, is what provides precision. The candidates file is discovery-owned — re-running `cadre discover` regenerates it and discards hand edits, loudly; `cadre setup`'s own discovery step never overwrites an existing file.
