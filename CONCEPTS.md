...
### Topology
How a fleet's lanes relate in time: **parallel** — independent and concurrent (the fan-out topology); **sequential** — each lane consumes *all preceding* lanes' successful output, accumulating context through the chain (the third stage sees both the first and second stages' output, not just the immediately preceding one); **iterative** — lanes run in rounds and see each other's output. Distinct from convergence: topology is execution order, convergence is output handling. Parallel and sequential are built; iterative is a future primitive.
