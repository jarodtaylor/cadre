...
### Result status
The explicit tri-state outcome of a completed fleet run. **SUCCESS** — the convergence step produced output (synthesize: synthesis present; collect: ≥1 specialist survived; judge: judge graded). **DEGRADED** — specialists survived but the convergence step ran and failed; a partial result is available (synthesize and judge only — collect is always SUCCESS or FAILED). **FAILED** — all specialists failed; convergence never ran.

The engine sets the status at every `run_fleet` return point. Two derived reads: `ok` (`status is SUCCESS`) is the exit-code signal for full success; `has_usable_output()` (`status is not FAILED`) covers both SUCCESS and DEGRADED. `synth_ok` and `judge_ok` remain as mode-specific detail for callers that need them; the run manifest records the `status` string alongside both.
