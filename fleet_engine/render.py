...
from fleet_engine.engine import DEFAULT_CALL_TIMEOUT, FleetResult, FleetStatus, CHAIN_STAGE_TOKEN_CAP
...
def render_fleet_preview(
    ...
    if config.topology == "sequential":
        ...
        out.append(f"  Inter-stage output cap: ~{CHAIN_STAGE_TOKEN_CAP:,} tokens (estimate)")
    ...
    if config.topology == "iterative":
        ...
        out.append(f"  Inter-round output cap: ~{CHAIN_STAGE_TOKEN_CAP:,} tokens (estimate)")
